...
knitr::include_graphics(system.file("rmd", "harrislogo.jpg", package = "cmfproperty"))
...
#main plots
results <- cmfproperty::plots(ratios, min_reporting_yr, max_reporting_yr, params$title)
...
This report presents a basic sales ratio study for `r paste0(params$title)`, based on user supplied data. Data was used for residential properties that sold between `r paste0(min_reporting_yr)` and `r paste0(max_reporting_yr)` and are classified as arm’s-length transactions utilizing the IAAO Standard. For each home that sold, we compute the sales ratio as the assessed value in place on January 1 of the sale year divided by the sale price. For more details, see the [Appendix](#appendix).
...
```{r iaao graphs}
iaao_rslt <- iaao_graphs(stats, ratios, min_reporting_yr, max_reporting_yr, params$region)
```
...
- [Click here](#regressions) to see how alternative measures of regressivity evaluated for `r paste0(params$region)`
...
all <- stats %>%
  dplyr::select(Year, N, med_sale, cod_with_ci, prd_with_ci, prb_with_ci)

kableExtra::kable(all, align = rep("c", 5),
      col.names = c("Tax Year", "Arms Length Sales", "Average Sale Price", "COD", "PRD", "PRB"),
...
Aside from the standard PRD and PRB tests recommended by the IAAO, several alternative metrics have been proposed by academic researchers [@Hodge]. Table 4.6.1 presents estimates of the most commonly used models.
